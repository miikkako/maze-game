            ...
            if self.pressbutton(screen, event, startbuttontext, 100, 450, 600, 100, BLUE, BRIGHTGREEN, GREY, PURPLE):
                if self.inputsize[0] != '' and self.inputsize[1] != '' and self.playername != '' and self.inputlevelnumber != '':
                    ...
                    if width >= 10 and width <= 48 and height >= 10 and height <= 30:
                        '''---------This part checks if there already is a file named after given name---------'''
                        write = False # check if playername already exists in saved files
                        try:
                            input = open(self.playername+'.ma', 'r')
                        except IOError:
                            write = True
                        '''-----------------Can be removed if overwrite is wanted to be alloved----------------'''
                        if write:
...
def main(programnotdone):
    ...
    lastportalxy = None
    # for saving the game
    gameIO = IO(playername = mainmenu.playername, levels = mainmenu.inputlevelnumber, y = mainmenu.inputsize[0], x = mainmenu.inputsize[1])
    print(mainmenu.playername)

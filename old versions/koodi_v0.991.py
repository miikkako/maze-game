...
wallcolor = BLUE; screencolor = WHITE; textcolor = BLACK; playercolor = BRIGHTRED; portalcolor = BRIGHTGREEN
game_name = "MAZEv1" # for savegame files
pixel = 15 # the width and height of game squares
...
class Mainmenu(object):
    ...
    def show_scoretable(self, screen, scores):
    ...
    def startscreen(self):
        ...
        highscoretext = Displaymessage('Highscores:', None, 45); highscoretext.setcenter(500, 200)
        ...
        while intro:  # select=1: play new game, select=2: load game
            ...
            self.show_scoretable(screen, scorelist) # blit hiscores if there are any
    ...
    def inputscreen(self):
        ...
        max_width = gamescreen_width//pixel
        max_height = gamescreen_height//pixel
    ...
    def loadgamescreen(self):
        ...
        while intro:
            ...
            if self.pressbutton(screen, event, loadbuttontext, 140, 300, 300, 100, GREY, BRIGHTBLUE, BLACK, WHITE):
                ...
                if not os.path.isfile(self.playername+'.ma') or self.playername == '': # check if there is a corresponing file
...
class IO(object):
    ...
    def __init__(self, playername = '', levels = 0, y = 0, x = 0, currentcoord = [0,0], score = 0, current_room_no = 0, passed_time = 0):
        self.matrixes = []
    ...
    def write_loadgame(self):
        ...
        file.write('#currentscore='+str(self.passed_time)+'\n')
        ...
        try:
            ...
            while allread == False and count < 100:
                ...
                if '=' in current_line:
                    ...
                    elif current_line_parts[0] == '#passedtime':
...
class Player(pygame.sprite.Sprite):
    ...
    def disappear(self, screen, color, current_room, coord):
        for i in range(pixel): # player disappearing at enter coordinates
            pygame.draw.rect(screen, color, (coord[0],coord[1], pixel, pixel))
            pygame.draw.rect(screen, self.color, (coord[0]+int(0.5*i),coord[1]+int(0.5*i), pixel-i, pixel-i))
            pygame.display.update()

    def appear(self, screen, current_room, coord):
        for i in range(pixel): # player appearing at lead coordinates
            pygame.draw.rect(screen, screencolor,(coord[0],coord[1], pixel, pixel))
            pygame.draw.rect(screen, self.color,(coord[0]+int(0.5*(pixel-i)),coord[1]+int(0.5*(pixel-i)), i, i))
            pygame.display.update()
    ...
    def jump(self, screen, current_room, fromcoord, tocoord):
        ...
        if current_room.wallmatrix.matrix[(self.rect.y+pixel)//pixel][self.rect.x//pixel] != 1:
            pygame.draw.rect(screen, screencolor, (self.rect.x, self.rect.y-pixel, pixel, pixel));pygame.display.update()
        if current_room.wallmatrix.matrix[(self.rect.y-pixel)//pixel][self.rect.x//pixel] != 1:
            pygame.draw.rect(screen, screencolor, (self.rect.x, self.rect.y+pixel, pixel, pixel));pygame.display.update()
        if current_room.wallmatrix.matrix[self.rect.y//pixel][(self.rect.x+pixel)//pixel] != 1:
            pygame.draw.rect(screen, screencolor, (self.rect.x-pixel, self.rect.y, pixel, pixel));pygame.display.update()
        if current_room.wallmatrix.matrix[self.rect.y//pixel][(self.rect.x-pixel)//pixel] != 1:
            pygame.draw.rect(screen, screencolor, (self.rect.x+pixel, self.rect.y, pixel, pixel));pygame.display.update()
...
class Level(object):
        ...
        for j in range(self.wallmatrix.height):
            for i in range(self.wallmatrix.width):
                if self.wallmatrix.matrix[j][i] == 1 or self.wallmatrix.matrix[j][i] == 8:
                    wall = Wall(i * pixel,j * pixel, pixel, pixel, self.color) # wall(x, y, width, height)
                    self.wall_list.add(wall) 
                elif self.wallmatrix.matrix[j][i] == 2:
                    self.start_xy = [i * pixel, j * pixel]
                elif self.wallmatrix.matrix[j][i] == 3:
                    self.portal_xy = [i * pixel, j * pixel]
                elif self.wallmatrix.matrix[j][i] == 4:
                    self.invisibleportal_enter_xy = [i * pixel, j * pixel]
                elif self.wallmatrix.matrix[j][i] == 5:
                    self.invisibleportal_lead_xy = [i * pixel, j * pixel]
    ...
    def showsolution(self, screen):
        ...
        while True:
            pygame.draw.rect(screen, playercolor, (ccx*pixel, ccy*pixel, pixel, pixel)); pygame.display.update()
...
def main():
    ...
    if mainmenu.select == 1: # newgame
        ...
        points = gameIO.score; passed_time = gameIO.passed_time
    ...
    if done == 1: # save the game
        ...
        if points <= 0: gratz = 'only'
        elif 100 > point >=0: gratz = 'nice'
        else: gratz = 'AMAZING'
        scoretext.text = 'Your Score is: '+gratz+str(int(points)); scoretext.size = width // 10
